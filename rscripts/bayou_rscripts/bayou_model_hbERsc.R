...
###hbER - bayou ###
{lnhbER_sc_prior <- make.prior(tree_prune, dists=list(dalpha="dhalfcauchy", dsig2="dhalfcauchy",dsb="dsb", dk="cdpois", dtheta="dnorm"), param=list(dalpha=list(scale=1), dsig2=list(scale=1), dk=list(lambda=15, kmax=200), dsb=list(bmax=1,prob=1), dtheta=list(mean=mean(lnhbER_sc_all), sd=sd(lnhbER_sc_all))),plot.prior=TRUE)}

#run1
lnhbER_sc_fit1 <- bayou.makeMCMC(tree_prune, lnhbER_sc_all,  model="OU", prior=lnhbER_sc_prior, file.dir="Rdata/bayou/Mustela/bayou_hbER_sc/", outname="lnhbER_sc_fit1", plot.freq=200000, ticker.freq=50000)
lnhbER_sc_fit1$run(1500000)
lnhbER_sc_chain1<-lnhbER_sc_fit1$load()
save(lnhbER_sc_chain1, file="bayou_lnhbER_sc_chain1.2.Rdata")
lnhbER_sc_chain1 <- set.burnin(lnhbER_sc_chain1, 0.3)
plot(lnhbER_sc_chain1)
lnhbER_sc_out1 <- summary(lnhbER_sc_chain1)
write.csv(lnhbER_sc_out1$branch.posteriors, file="lnhbER_sc_branch.posteriors_run1.csv")
write.csv(lnhbER_sc_out1$statistics, file="lnhbER_sc_statistics_run1.csv")


#run2
lnhbER_sc_fit2 <- bayou.makeMCMC(tree_prune, lnhbER_sc_all, model="OU", prior=lnhbER_sc_prior, file.dir="Rdata/bayou/Mustela/bayou_hbER_sc/", outname="lnhbER_sc_fit2", plot.freq=200000, ticker.freq=50000)
lnhbER_sc_fit2$run(1500000)
lnhbER_sc_chain2<-lnhbER_sc_fit2$load()
save(lnhbER_sc_chain2, file="bayou_lnhbER_sc_chain2.Rdata")
lnhbER_sc_chain2 <- set.burnin(lnhbER_sc_chain2, 0.3)
plot(lnhbER_sc_chain2)
lnhbER_sc_out2 <- summary(lnhbER_sc_chain2)
write.csv(lnhbER_sc_out2$branch.posteriors, file="lnhbER_sc_branch.posteriors_run2.csv")
write.csv(lnhbER_sc_out2$statistics, file="lnhbER_sc_statistics_run2.csv")

load("Rdata/bayou/Mustela/bayou_lnhbER_sc_chain1.2.Rdata")
pdf(width=5, height =7, file="bayou_hbER_shifts.pdf")
plotSimmap.mcmc(lnhbER_sc_chain1, burnin=0.3, lwd=5, edge.type="regimes", pal=colorRampPalette(c("darkred", "white", "blue")), show.tip.label=TRUE, cex = .25, pp.labels = TRUE, pp.cutoff = 0.5)
dev.off()

pdf(width=5, height =7, file="bayou_hbER_opt.pdf")
plotBranchHeatMap(tree_prune, lnhbER_sc_chain1, "theta", burnin = 0.3, pal = colorRampPalette(c("darkred", "yellow", "blue")), cex=.25, lwd =10)
dev.off()

load("Rdata/bayou/Mustela/bayou_lnhbER_sc_chain2.2.Rdata")
pdf(width=5, height =7, file="bayou_hbER_shifts.pdf")
plotSimmap.mcmc(lnhbER_sc_chain2, burnin=0.3, lwd=5, edge.type="regimes", pal=colorRampPalette(c("darkred", "white", "blue")), show.tip.label=TRUE, cex = .25, pp.labels = TRUE, pp.cutoff = 0.5)
dev.off()
plotBranchHeatMap(tree_prune, lnhbER_sc_chain2, "theta", burnin = 0.3, pal = colorRampPalette(c("darkred", "white", "blue")), cex=.25, lwd =10)

#Convergence check
lnhbER_sc_RlnL <- gelman.R("lnL", chain1=lnhbER_sc_chain1, chain2=lnhbER_sc_chain2, plot=TRUE, type="n", ylim=c(0.9, 2))
lnhbER_sc_Ralpha <- gelman.R("alpha", chain1=lnhbER_sc_chain1, chain2=lnhbER_sc_chain2, plot=TRUE, type="n", ylim=c(0.9, 2))

lnhbER_sc_L1 <- Lposterior(lnhbER_sc_chain1,tree_prune, burnin=0.3)
lnhbER_sc_L2 <- Lposterior(lnhbER_sc_chain2,tree_prune, burnin=0.3)
plot(lnhbER_sc_L1$pp,lnhbER_sc_L2$pp, xlim=c(0,.3), ylim=c(0,.3), xlab="lnhbER_sc_Chain 1", ylab="lnhbER_sc_Chain 2")
curve(1*x, add=TRUE, lty=2)
